...
右上角展开菜单可选择「自由发呆」「指尖拨球」「浅窝托盘」。自由发呆保留倾斜观察；指尖拨球可以沿盒底拖动、拨出单颗球；浅窝托盘提供三颗网球和三个真实凹窝，继续倾斜可以滚出。回到其他模式恢复之前的球体配置。玩法不包含倒计时、分数或失败惩罚。

倾斜灵敏度提供自然、灵敏、更灵敏三档，默认灵敏；增益改变平面运动力度，重力方向始终遵循屏幕低的一边。球体仍保留惯性，运动中的球需要减速后才会反向，这与输入等待不同。

碰撞音和轻触反馈独立开关；声音默认关闭，轻触默认开启。原生播放根据撞击强度和球种变化，并合并、限频，暂停与后台停止反馈。合成音效可通过 `python3 tools/generate-impact-audio.py` 重建，不依赖外部素材。

Android 逐帧读取最新传感器值，避免将旧重力值排入繁忙的 WebView；iOS 每帧读取 Core Motion 最新样本，保留单条在途传递。场景测试接口记录样本年龄、桥接往返和帧间隔；模拟器与主机测试不能证明真实手机端到端延迟。

Android 0.1.1 起可在菜单内检查、下载并确认安装后续版本。iOS Personal Team 版可检查新版和查看 Mac 更新方法，仍需在自己的 Mac 上通过 Xcode 签名覆盖安装；免费签名期限不会被版本检查延长。TestFlight 分发尚未启用。
