...
(() => {
  ...
  function changeBoundaries(width,height){
    ...
    dynamics=createBallPhysics({THREE,halfW,halfH,corner:activeCorner,depth});
    if(interior){scene.remove(interior.group);interior.dispose();}
    interior=buildPocketInterior(THREE,{width:innerW,height:innerH,corner:activeCorner,depth});interior.setTheme(false);scene.add(interior.group);
  ...
  function updateCount(){root.querySelector('output').textContent=balls.length+' 颗';root.querySelector('.ib-less').disabled=balls.length<=1;root.querySelector('.ib-more').disabled=balls.length>=32;canvas.setAttribute('aria-label',`${balls.length} 个运动球随手机倾斜滚动和碰撞`);}
  function updateSelection(){root.querySelectorAll('[data-type]').forEach(el=>el.setAttribute('aria-pressed',String(el.dataset.type===selected)));}
  ...
  function disposeBall(b){scene.remove(b.mesh,b.shade);b.shade.material.dispose();}
  ...
  function addBall(){
    if(balls.length>=32)return;const i=balls.length,type=selected==='mixed'?names[(i+Math.floor(i/4))%4]:selected,b=newBall(type);
  ...
  function serialize(){return JSON.stringify({schema:1,selected,paused,radius:baseRadius,effectiveRadius,bounds:bounds(),balls:balls.map(b=>({type:b.type,x:b.x,y:b.y,z:b.z,vx:b.vx,vy:b.vy,vz:b.vz,wx:b.wx,wy:b.wy,wz:b.wz,q:b.mesh.quaternion.toArray()}))});}
  ...
  function loadState(){
    ...
    }catch(e){clearBalls();return false;}
  }
  ...
  root.querySelector('.ib-less').addEventListener('click',()=>{if(balls.length>1){disposeBall(balls.pop());fitBallRadii();safeSeparate();updateCount();scheduleSave();}});
  root.querySelector('.ib-reset').addEventListener('click',()=>{packBalls();scheduleSave();});
  root.querySelectorAll('[data-type]').forEach(el=>el.addEventListener('click',()=>{selected=el.dataset.type;updateSelection();balls.forEach((b,i)=>{b.type=selected==='mixed'?names[(i+Math.floor(i/4))%4]:selected;const r=radiusFor(b.type);b.z+=r-b.r;b.r=r;b.mesh.material=materials[b.type];dynamics.configureBall(b,b.type);dynamics.constrain(b);});safeSeparate();scheduleSave();}));
